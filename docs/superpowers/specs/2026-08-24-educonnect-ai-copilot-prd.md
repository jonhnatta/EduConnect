...
Traces exportarão somente metadados permitidos, como identificadores técnicos, categorias, decisões, booleanos, números e comprimentos. Texto livre de entrada ou saída, nomes, CPF, dados de alunos e conteúdo pedagógico não será exportado em texto puro, inclusive quando flags de amostragem estiverem habilitadas. Uma futura captura de conteúdo exigirá um payload tipado, explicitamente classificado e previamente revisado, sem aceitar strings ou objetos arbitrários. A indisponibilidade do Langfuse será reportável para alertas, mas não fará o readiness retornar 503.
